...
class Goods extends Controller
{
    ...
    public function add()
    {
        if (!$this->request->isAjax()) {
            // 商品分类
            $catgory = Category::getWXCacheTree();

            // 配送模板
            $delivery = Delivery::getAll();
            //VIP积分
            $vip = VipjifenModel::getAll();
            return $this->fetch('add', compact('catgory', 'delivery','vip'));
        }
        $model = new GoodsModel;
    ...
    {
        // 商品详情
        $model = GoodsModel::detail1($goods_id);
        if (!$this->request->isAjax()) {
            // 商品分类
            $catgory = Category::getWXCacheTree();//返回数组
            // 配送模板
            $delivery = Delivery::getAll();//返回对象
            // 多规格信息
            //VIP积分
            $vip = VipjifenModel::getAll();
            $specData = 'null';
            ...
            return $this->fetch('edit', compact('model', 'catgory', 'delivery', 'specData','vip'));
        }
        // 更新记录
        // print_r($this->postData('goods'));
        // exit;
