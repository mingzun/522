...
use think\Cache;
use app\common\model\Tcmarket as Tcsc;
 ...
 */
class TcscCategory extends BaseModel
{
    protected $name = 'Tcsc_category';
    ...
    public static function getALL()
    {
        ...
        if (!Cache::get('tcsc_category' . $model::$wxapp_id)) {
           ...
           /* var_dump($data);exit;*/
            ...
            Cache::set('tcsc_category' . $model::$wxapp_id, compact('all', 'tree'));
        }
        return Cache::get('tcsc_category' . $model::$wxapp_id);
    }
...
//        }
    ...
    {
        return Cache::rm('tcsc_category' . self::$wxapp_id);
    }
    public function remove($category_id)
    {
        // 判断是否存在商品
        if ($goodsCount = (new Tcsc)->where('category_id','=',$category_id)->count()) {
