...
class Order extends OrderModel
{
    ...
    public function getBuyNowlbzx($user, $goods_id, $goods_num, $goods_sku_id)
    {
        // 商品信息
        /* @var Goods $goods */
        $goods = LbzxModel::detail($goods_id)->toArray();
        // 商品单价
        $goods['goods_price'] = $goods['money'];
        // 商品总价
        $goods['total_num'] = $goods_num;
        $goods['total_price'] = $totalPrice = bcmul($goods['goods_price'], $goods_num, 2);
        // 当前用户收货城市id
    ...
    public function add($user_id, $order)
    {
     ...
     public function addlbzx($user_id, $order)
    {
        if (empty($order['address'])) {
            $this->error = '请先选择收货地址';
            return false;
