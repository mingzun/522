...
class Order extends Controller
{
    ...
    public function buyNow($goods_id, $goods_num, $goods_sku_id)
    {
        ...
        public function buyNowlbzx($goods_id, $goods_num, $goods_sku_id)
    {
        // 商品结算信息
        $model = new OrderModel;
        $order = $model->getBuyNowlbzx($this->user, $goods_id, $goods_num, $goods_sku_id);
        if (!$this->request->isPost()) {
            return $this->renderSuccess($order);
        }
        if ($model->hasError()) {
        ...
        // 创建订单
        if ($model->addlbzx($this->user['user_id'], $order)) {
            return $this->renderSuccess([
                'payment' => $this->wxPay($model['order_no'], $this->user['open_id']
                    , $order['order_pay_price']),
                'order_id' => $model['order_id']
